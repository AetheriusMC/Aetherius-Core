...
def detect_terminal_capabilities() -> dict:
    ...
    if capabilities["interactive"]:
        ...
        capabilities["color_support"] = (
            ...
            term in ["xterm", "xterm-256color", "screen", "tmux"]
        )
...
class ConsoleManager:
    ...
    def create_console(self, force_type: Optional[str] = None):
        ...
        if force_type:
            self.rich_console.print(f"[yellow]Note: Console type '{force_type}' ignored - using unified console[/yellow]")
        ...
        try:
            ...
            if not server_instance:
                ...
                try:
                    ...
                    self.rich_console.print(f"[blue]Connected to running server[/blue]")
                except Exception as e:
                    self.rich_console.print(f"[yellow]No running server found: {e}[/yellow]")
            
            console = SimpleConsole(server_instance)
            self.selected_console = console
            self.logger.info(f"Created simplified unified console")
            
            self.rich_console.print(f"[green]Using simplified unified console[/green]")
    ...
    def diagnose_issues(self) -> list:
        ...
        if self.capabilities.width < 80:
            issues.append(f"Terminal width ({self.capabilities.width}) is narrow - some features may not display properly")
        
        if self.capabilities.height < 24:
            issues.append(f"Terminal height ({self.capabilities.height}) is short - display may be cramped")
    ...
    def show_console_info(self):
        ...
        self.rich_console.print(f"[cyan]Aetherius Console Manager[/cyan]")
        self.rich_console.print(f"[yellow]Console Type:[/yellow] Unified (with automatic mode detection)")
        self.rich_console.print(f"[yellow]Capabilities:[/yellow]")
        self.rich_console.print(f"  Color Support: {'[green]✓[/green]' if self.capabilities.has_color else '[red]✗[/red]'}")
        self.rich_console.print(f"  Unicode Support: {'[green]✓[/green]' if self.capabilities.has_unicode else '[red]✗[/red]'}")
        self.rich_console.print(f"  Raw Mode: {'[green]✓[/green]' if self.capabilities.has_raw_mode else '[red]✗[/red]'}")
        self.rich_console.print(f"  Interactive: {'[green]✓[/green]' if self.capabilities.is_interactive else '[red]✗[/red]'}")
        self.rich_console.print(f"  Terminal Size: {self.capabilities.width}x{self.capabilities.height}")
...
def should_use_enhanced_console() -> bool:
    ...
    return (
        caps["interactive"] and 
        caps["color_support"] and 
        caps["raw_mode"] and
        caps["size_detection"]
    )
...
async def start_console(server_wrapper, command_handler: Optional[Callable] = None, 
                       console_type: Optional[str] = None) -> None:
    ...
    try:
        ...
        console.print("[yellow]The simplified console handles all functionality automatically[/yellow]")