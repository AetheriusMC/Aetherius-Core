...
class PluginState:
        ...
        try:
            if self.state_file.exists():
                with open(self.state_file, 'r', encoding='utf-8') as f:
                    ...
                    logger.debug(f"Loaded plugin state: {len(self.loaded_plugins)} loaded, {len(self.enabled_plugins)} enabled")
    ...
    def save_state(self) -> None:
        ...
        try:
            ...
            logger.debug(f"Saved plugin state: {len(self.loaded_plugins)} loaded, {len(self.enabled_plugins)} enabled")
        except Exception as e:
