...
class OutputCapture:
    ...
    def __init__(self):
        ...
        self.command_patterns = {
            ...
            'time': [
                r'Set the time to',
                r'Added \d+ to the time'
            ],
            'weather': [
                r'Set the weather to',
                r'Weather set to'
            ],
            'difficulty': [
                r'Set the difficulty to',
                r'Difficulty set to'
            ]
        }
    ...
    def start_capture(self, command_id: str, command: str) -> None:
        ...
        logger.debug(f"Started output capture for command: {command} (ID: {command_id})")
    ...
    def process_line(self, line: str) -> None:
        ...
        for command_id, capture in list(self.active_captures.items()):
            if self._is_line_relevant(capture.command, clean_line):
                ...
                logger.debug(f"Captured line for command {capture.command}: {clean_line}")
    ...
    def finish_capture(self, command_id: str) -> Optional[str]:
        ...
        logger.debug(f"Finished capture for command {capture.command}, captured {len(capture.lines)} lines")
    ...
    def _is_line_relevant(self, command: str, line: str) -> bool:
        ...
        if base_command == 'list':
            ...
            if re.search(r'^[a-zA-Z_][a-zA-Z0-9_]{2,15}(,\s*[a-zA-Z_][a-zA-Z0-9_]{2,15})*$', line):
                return True
    ...
    def _cleanup_expired(self) -> None:
        ...
        expired_ids = [
            command_id for command_id, capture in self.active_captures.items()
...
_output_capture: Optional[OutputCapture] = None
