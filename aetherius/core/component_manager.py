...
class ComponentManager:
        ...
        try:
            ...
            for field in required_fields:
                if field not in data:
                    logger.error(f"Component {component_dir.name} missing required field: {field}")
                    return None
            
            # 创建ComponentInfo对象
            info = ComponentInfo(**data)
    ...
    async def load_component(self, component_name: str) -> bool:
        ...
        try:
            ...
            if missing_deps:
                logger.error(f"Component {component_name} missing dependencies: {missing_deps}")
                self._failed_components[component_name] = f"Missing dependencies: {missing_deps}"
                return False
            ...
            if not component_class:
                for name, obj in inspect.getmembers(module):
                    if (inspect.isclass(obj) and 
                        ...
                        obj != Component and obj != WebComponent):
                        component_class = obj
                        break
            ...
            logger.info(f"Successfully loaded component: {component_name} v{info.version}")
            
            # 发送组件加载事件
            if hasattr(self.core, 'event_manager'):
    ...
    def _load_component_config(self, component_dir: Path, info: ComponentInfo) -> Dict[str, Any]:
        """
        加载组件配置
    ...
    def get_web_component(self, component_name: str) -> Optional[WebComponent]:
        """获取Web组件实例"""
