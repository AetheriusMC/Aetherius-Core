...
class CommandQueue:
    ...
    async def wait_for_completion(self, command_id: str, timeout: float = 30.0) -> Dict[str, Any]:
        """
        ...
        try:
            for command_file in self.pending_dir.glob("*.json"):
                try:
                    ...
                    if time.time() - command_data['timestamp'] > command_data.get('timeout', 30.0):
    ...
    def mark_command_completed(self, command_id: str, success: bool, error: Optional[str] = None, output: Optional[str] = None) -> None:
        ...
        try:
            ...
            logger.debug(f"Marked command as completed: {command_id} (success: {success})")
