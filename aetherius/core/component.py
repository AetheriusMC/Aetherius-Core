...
class Component(ABC):
    ...
    @data_folder.setter
    def data_folder(self, path: Union[str, Path]):
        """设置组件数据目录"""
    ...
    def register_event_handler(self, event_name: str, handler_method: str):
        ...
        if hasattr(self, handler_method):
            ...
            if self.core and hasattr(self.core, 'event_manager'):
                ...
                self.logger.debug(f"Registered event handler: {event_name} -> {handler_method}")
...
class WebComponent(Component):
    ...
    def add_api_endpoint(self, path: str, handler, methods: List[str] = None):
        """
        添加API端点
