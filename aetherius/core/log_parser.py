...
from typing import Dict, List, Optional, Pattern, Type, Union, Any
import yaml

from .events import (
    BaseEvent, PlayerJoinEvent, PlayerLeaveEvent, PlayerChatEvent,
    PlayerDeathEvent, PlayerAdvancementEvent, ServerStartedEvent,
    ServerStoppingEvent, LogLineEvent, UnknownLogEvent, TickTimeEvent,
    LagSpikeEvent
)
...
class LogPattern:
    ...
    def try_parse(self, line: str, timestamp: Optional[datetime] = None) -> Optional[BaseEvent]:
        """
        ...
        if self.condition:
            try:
                ...
                if not eval(self.condition, {"match": match, "line": line, "data": event_data}):
                    return None
        ...
        for pattern in self.patterns:
            ...
            try:
                ...
                if event:
                    events.append(event)
                    parsed = True
                    logger.debug(f"Parsed line with pattern '{pattern.name}': {type(event).__name__}")
