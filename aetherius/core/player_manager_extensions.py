...
from .player_data import PlayerDataManager, PlayerLocation, PlayerStats, PlayerInventory
...
class PlayerManagerExtensions:
    ...
    def __init__(self, player_data_manager: PlayerDataManager, database_path: str = None):
        """
        初始化玩家管理器扩展
    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_sessions_player ON player_sessions (player_uuid)')
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_actions_player ON player_actions (player_uuid)')
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_actions_type ON player_actions (action_type)')
    ...
    async def get_player_info(self, player_identifier: str, use_cache: bool = True) -> Optional[ExtendedPlayerInfo]:
        """
        获取扩展的玩家信息
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if len(player_identifier) == 36 and '-' in player_identifier:
                    # UUID格式
                    cursor.execute('SELECT * FROM players WHERE uuid = ?', (player_identifier,))
                else:
                    # 玩家名格式
                    cursor.execute('SELECT * FROM players WHERE name = ?', (player_identifier,))
                ...
                # 获取基础玩家数据
                basic_data = await self.player_manager.get_player_data(player_data['uuid'])
                ...
                player_info = ExtendedPlayerInfo(
                    ...
                    first_join=datetime.fromisoformat(player_data['first_join']) if player_data['first_join'] else datetime.now(),
                    last_join=datetime.fromisoformat(player_data['last_join']) if player_data['last_join'] else datetime.now(),
                    last_quit=datetime.fromisoformat(player_data['last_quit']) if player_data['last_quit'] else datetime.now(),
                    total_playtime=player_data['total_playtime'] or 0,
                    session_playtime=self._calculate_session_playtime(player_data['uuid']),
                    location=basic_data.location if basic_data else PlayerLocation(0, 0, 0, 'overworld'),
                    ...
                    permission_groups=json.loads(player_data['permission_groups'] or '[]'),
    ...
    def _get_player_statistics(self, player_uuid: str) -> Dict[str, int]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute('SELECT stat_name, stat_value FROM player_stats WHERE player_uuid = ?', (player_uuid,))
    ...
    async def record_player_join(self, player_name: str, player_uuid: str, ip_address: str) -> bool:
        """
        记录玩家加入
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, 
                        ...
                        ?, ?)
                ''', (player_uuid, player_name, ip_address, player_uuid, join_time, join_time, join_time))
    ...
    async def _record_action(self, player_uuid: str, action_type: str, details: Dict[str, Any], location: PlayerLocation = None):
        """记录玩家行为"""
        try:
            action = PlayerAction(
                player_uuid=player_uuid,
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    player_uuid, action_type, action.timestamp, json.dumps(details),
    ...
    async def get_player_history(self, player_uuid: str, 
                               action_types: List[str] = None,
                               limit: int = 100,
                               days: int = 30) -> List[PlayerAction]:
        """
        获取玩家历史行为
    ...
    async def get_server_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute('SELECT COUNT(*) FROM players WHERE DATE(first_join) = ?', (today,))
                new_players_today = cursor.fetchone()[0]
                
                # 本周活跃玩家数
                week_ago = datetime.now() - timedelta(days=7)
                cursor.execute('SELECT COUNT(*) FROM players WHERE last_join >= ?', (week_ago,))
                active_players_week = cursor.fetchone()[0]
                
                # 平均游戏时间
                cursor.execute('SELECT AVG(total_playtime) FROM players WHERE total_playtime > 0')
    ...
    async def search_players(self, query: str, 
                           ...
                           limit: int = 50) -> List[ExtendedPlayerInfo]:
        """
        搜索玩家
    ...
    async def ban_player(self, player_uuid: str, reason: str = "", admin_uuid: str = ""):
        """封禁玩家"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
            ...
            await self._record_action(player_uuid, 'ban', {
                'reason': reason,
    ...
    async def unban_player(self, player_uuid: str, admin_uuid: str = ""):
        ...
        try:
            ...
            await self._record_action(player_uuid, 'unban', {
                'admin_uuid': admin_uuid
            })
