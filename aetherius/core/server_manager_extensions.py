...
class ServerManagerExtensions:
    ...
    async def get_server_status(self, use_cache: bool = True) -> ServerStatus:
        ...
        # 检查缓存
        if (use_cache and self._cached_status and self._status_cache_time and
            (now - self._status_cache_time).total_seconds() < self._status_cache_ttl):
            return self._cached_status
        
        try:
            # 基本状态
            is_running = self.server_wrapper.is_alive
            is_starting = hasattr(self.server_wrapper, '_starting') and self.server_wrapper._starting
            is_stopping = hasattr(self.server_wrapper, '_stopping') and self.server_wrapper._stopping
            ...
            max_players = self.config.max_players if hasattr(self.config, 'max_players') else 20
            ...
            status = ServerStatus(
                ...
                simulation_distance=int(server_properties.get('simulation-distance', 10))
            )
    ...
    async def execute_command_with_result(self, command: str, timeout: float = 30.0) -> Dict[str, Any]:
        """
        执行命令并等待结果
        ...
        try:
            ...
            result = await self.server_wrapper.command_queue.wait_for_completion(command_id, timeout)
    ...
    async def get_command_history(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        Returns:
            命令历史列表
        """
        return self._command_history[-limit:] if len(self._command_history) > limit else self._command_history
